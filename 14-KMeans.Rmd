...
1. Write an R script to select a k-means sample of size 20, using scaled distances. Compute the Mean Squared Shortest **Scaled** Distance (MSSSD) of the k-means sample   
2. Use [FuzME](https://sydney.edu.au/agriculture/pal/software/fuzme.shtml) to compute a fuzzy clustering for 20 classes, using the five covariates of Hunter Valley in clustering the grid cells. The input file for FuzME is `grdHunterValley4Practicals4FuzME.csv`. Use scaled distances (tic in FuzME in box Metric Distance Diagonal). Use 1.3 as fuzzy exponent, and set maximum number of iterations to 1000    
3. Write an R script to select the fuzzy k-means sample for this fuzzy clustering, and compute the Mean Squared Shortest **Scaled** Distance (MSSSD) in covariate space (spanned by the five covariates) of the sample using function `rdist`     
4. Repeat this for Mahalanobis distances   
5. Compare the MSSSD's of the k-means and the two fuzzy k-means samples  
