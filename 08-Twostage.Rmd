...
1. Select randomly one ssu from the list with $M=\sum_{i=1}^N M_i$ ssu's, and determine the psu of the selected ssu 
...
The `R` code below shows the selection of a two-stage random sample from Voorst. Primary units are 24 blocks of 0.5 km $\times$ 0.5 km. Note that these blocks have unequal size, i.e. the number of pixels within the blocks differs due to build-up area, roads *et cetera*.  Four times a psu is selected, with probabilities proportional to size and with replacement. The second stage sample size $m_i$ equals 10 for all psu's. These units ssu's are selected by SIR. 
...
```{r,echo=F,out.width='100%', fig.asp=.333, fig.cap="Two-stage random sample from Voorst"}
...
If the psu's are selected with replacement and with probabilities proportional to size (ppswr), then the population mean can be estimated by the simple estimator:
\begin{equation}
...
#### Exercise ([TwoStage.R](https://github.com/DickBrus/SpatialSamplingwithR/blob/master/Exercises/TwoStage.R)) {-}
1. Write an R script to select a two-stage random sample from Voorst. Select four times a psu by ppswr, and ten points (ssu's) per psu-draw by SI.  
2. Estimate the population mean, and its standard error  
3. Compare the standard error with that of the Horvitz-Thompson estimator for simple random sampling of the same size; explain the difference   
4. Is the standard error of the estimated mean for $10 \times 4$ units larger or smaller than with $4 \times 10$?  
