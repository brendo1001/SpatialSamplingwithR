...
```{r grid}
mysample<-spsample(x=Voorst,type="regular",cellsize=c(200,200),offset=c(0.5,0.5))
mysample<-as(mysample,"data.frame")
```

Figure \@ref(fig:grdVoorst) shows the selected square grid.
...
The number of gridpoints equals ```r n```. Nodes of the square grid in parts of the area not belonging to the population of interest, such as build-up areas and roads, are discarded. As a consequence,there are some empty spaces, for instance in the middle of the study area.
